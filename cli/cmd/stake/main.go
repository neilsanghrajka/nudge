...
import (
	...
	"github.com/neilsanghrajka/stake-ai/cli/internal/motivation"
	"github.com/neilsanghrajka/stake-ai/cli/internal/punishment"
	"github.com/neilsanghrajka/stake-ai/cli/internal/secrets"
	"github.com/neilsanghrajka/stake-ai/cli/internal/store"
	"github.com/neilsanghrajka/stake-ai/cli/internal/task"
)
...
func taskAdd(args []string) {
	...
	if desc == "" {
		exitErr("task add", "missing --desc. Example: stake task add --desc 'Finish PR' --duration 30 --why 'Team waiting'", "MISSING_DESC")
		return
	}
...
func taskComplete(args []string) {
	if len(args) == 0 {
		exitErr("task complete", "missing task_id. Example: stake task complete task-1", "MISSING_ID")
		return
	}
...
func taskFail(args []string) {
	if len(args) == 0 {
		exitErr("task fail", "missing task_id. Example: stake task fail task-1", "MISSING_ID")
		return
	}
...
func handleSecrets(args []string) {
	if len(args) == 0 {
		fmt.Println("Usage: stake secrets <add|list|pick> [flags]")
		return
	}
	switch args[0] {
	case "add":
...
func secretsAdd(args []string) {
	...
	if secret == "" {
		exitErr("secrets add", "missing --secret. Example: stake secrets add --secret 'I sleep with a nightlight' --severity medium", "MISSING_SECRET")
		return
	}
...
func secretsList() {
	...
	if !jsonMode {
		if len(ss.Secrets) == 0 {
			fmt.Println("No secrets. Add one: stake secrets add --secret '...'")
			return
		}
...
func handleMotivation(args []string) {
	if len(args) == 0 {
		fmt.Println("Usage: stake motivation <list|add> [flags]")
		return
	}
	switch args[0] {
	case "list":
...
func handlePunishment(args []string) {
	if len(args) == 0 {
		fmt.Println("Usage: stake punishment <list|setup|health> [flags]")
		return
	}
	switch args[0] {
	case "list":
...
func punishmentSetup(args []string) {
	if len(args) == 0 {
		exitErr("punishment setup", "missing action name. Example: stake punishment setup post_to_beeper_whatsapp --token abc123", "MISSING_ACTION")
		return
	}
	actionName := args[0]
	cfg := config.Load()
...
func handleConfig(args []string) {
	if len(args) == 0 {
		fmt.Println("Usage: stake config <show|set> [flags]")
		return
	}
	switch args[0] {
	case "show":
	...
	case "set":
		if len(args) < 3 {
			exitErr("config set", "usage: stake config set <key> <value>", "MISSING_ARGS")
			return
		}
		cfg := config.Load()
...
func printUsage() {
	fmt.Print(`stake — motivation through consequences

Usage: stake <command> [flags]
...
Examples:
  stake task add --desc "Finish PR" --duration 30 --why "Team waiting"
  stake task complete task-1
  stake secrets add --secret "I sleep with a nightlight" --severity medium
  stake punishment setup post_to_beeper_whatsapp --token abc123

Run 'stake <command> --help' for command-specific help.
`)
}

func printTaskHelp() {
	fmt.Print(`stake task — manage accountability tasks

Usage: stake task <action> [flags]
...
Examples:
  stake task add --desc "Ship feature" --duration 60 --why "Demo tomorrow" --secret-id s-1
  stake task add --desc "Write tests" --duration 30 --punishment post_to_beeper_whatsapp --targets "!room:..."
  stake task complete task-1
  stake task status
  stake task list --all
