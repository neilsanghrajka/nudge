...
### Automated Execution
Warnings and punishments are executed directly by the nudge CLI — not by the agent. Prefer one-shot openclaw cron jobs generated by the CLI itself:
```bash
# Create the task and get the exact cron schedule
nudge task add --json ...

# Then, for each entry in the returned data.crons array:
# - create an openclaw cron with that name
# - schedule it at the given timestamp via --at
# - set --delete-after-run
# - run: nudge task check
```
After running `nudge task add --json`, parse the `data.crons` array and create openclaw cron jobs. After complete/cancel/fail, parse `data.cancel_crons` and remove those cron jobs.

The daemon is still available for short sprints:
```bash
nudge task daemon --interval 30
```

Each cron job should still run the same local command: `nudge task check`. The `check` command handles all active tasks, firing any overdue warnings and auto-failing past-deadline tasks. This is why the cron jobs are safe and idempotent.
